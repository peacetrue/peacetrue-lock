...
public class RedisLockService implements LockService {
    ...
    @Setter
    private long expiry = 1000 * 10; // 10 seconds
    ...
    public Lock create(String name) {
        return create(name, expiry);
    }

    @Override
    public Lock create(String name, long expiry) throws LockExistsException {
        String stored = getValue(name);
        if (stored != null) {
            throw new LockExistsException(name);
        }
    ...
    public Lock refresh(String name, String value) {
        //TODO the expire not matched for create
        String key = keyForName(name);
        String stored = getValue(name);
        if (stored != null && value.equals(stored)) {
            Date expires = new Date(System.currentTimeMillis() + expiry);
